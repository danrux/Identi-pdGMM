...
def generate_ball_dataset(n_balls, means, mask_values, Sigma, mask_prob, sample_num):
    Z = []
    X = []

    masks = np.random.rand(sample_num, n_balls)
    masks = (masks >= mask_prob).astype(int)

    for j in range(sample_num):
        while True:
            z = np.random.multivariate_normal(np.zeros(2), Sigma, n_balls)
            z = z + means
            if np.all(z <= 0.95) and np.all(z >= 0.05):
                break
